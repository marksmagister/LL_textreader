...
def set_term(req: TermUpdate, user: User = CurrentUser) -> dict[str, str | int | None]:
    ...
    with connect() as conn:
        ...
        conn.execute(
            ...
            """,
            (user.id, req.lang, req.lemma, req.pos, req.status, req.note, req.context),
        )
        if req.surface:
            conn.execute(
                """
                ...
                """,
                (user.id, req.lang, req.lemma, req.pos, req.surface.casefold()),
            )
        if req.lesson_id is not None and req.page is not None:
            ...
            conn.execute(
                ...
                VALUES (?,?,?,?,?,?)
                """,
                (user.id, req.lang, req.lemma, req.pos, req.lesson_id, req.page),
            )
        ...
        if bucket(was["status"] if was else None) != bucket(req.status):
            refresh_for_words(conn, user.id, req.lang, [(req.lemma, req.pos)])
...
def set_override(req: OverrideRequest, user: User = CurrentUser) -> dict[str, str]:
