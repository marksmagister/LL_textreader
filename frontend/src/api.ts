 ...
 *
 *  It also carries `languages`, so a signed-in reader needs no separate
 *  `/api/health` round trip to know what the server can read. */
 ...
 *  populated from the server rather than from a constant in the bundle: what you
 *  can read is a fact about the server's models, not about the frontend. */
